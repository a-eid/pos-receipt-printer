...
function getDownloadUrl(asset) {
  const owner = 'a-eid';
  const repo = 'pos-receipt-printer';
      ...
      if (response.statusCode && response.statusCode >= 300 && response.statusCode < 400 && response.headers.location) {
        ...
        https.get(response.headers.location, (res2) => {
          if (res2.statusCode !== 200) {
            file.close();
            fs.unlink(dest, () => {});
            return reject(new Error(`Failed to download binary. HTTP ${res2.statusCode} for redirected URL`));
          }
          res2.pipe(file);
...
(async () => {
  try {
    ...
    process.stdout.write(`Downloading prebuilt binary for ${platform}-${arch}:\n  ${url}\n`);
